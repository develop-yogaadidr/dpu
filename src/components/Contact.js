...
import {fetchContact} from '../actions/PagesAction';
...
class Contact extends Component {
  ...
  componentDidMount() {
    this.setState({
      data: this.props.dataContact.text,
    });
    this.getDataUser();
  }
  ...
  getDataUser = async () => {
    ...
    if (this._isMounted) {
      this.setState({token}, () => {
        this.props.fetchContact(token);
      });
    }
  };
  componentDidUpdate(prevProps) {
    if (
      JSON.stringify(this.props.dataContact) !==
      JSON.stringify(prevProps.dataContact)
    ) {
      this.setState({
        data: this.props.dataContact.text,
      });
      console.log(this.props.dataContact);
    }
  }
...
const mapStateToProps = ({page}) => {
  return {
    ...
    dataContact: page.dataContact,
  };
};

mapDispatchToProps = dispatch => {
  return {
    fetchContact: token => dispatch(fetchContact(token)),
  };
};

...
import {fetchFaq} from '../actions/PagesAction';
...
class Faq extends Component {
  ...
  getDataUser = async () => {
    ...
    if (this._isMounted) {
      this.setState({token}, () => {
        this.props.fetchFaq(token);
      });
    }
  };
  componentDidUpdate(prevProps) {
    if (
      JSON.stringify(this.props.dataFaq) !== JSON.stringify(prevProps.dataFaq)
    ) {
      this.setState({
        data: this.props.dataFaq.text,
      });
      console.log(this.props.dataFaq);
    }
  }
...
const mapStateToProps = ({page}) => {
  return {
    ...
    dataFaq: page.dataFaq,
  };
};

mapDispatchToProps = dispatch => {
  return {
    fetchFaq: token => dispatch(fetchFaq(token)),
  };
};
